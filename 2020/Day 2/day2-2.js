...
4-5 s: rsssw
2-14 c: jckbwnnlkcmvnwtj`.split('\n');

let countValidPasswords = getCountValidPasswords(passwordRules);
...
function getCountValidPasswords(passwordRules) {
    ...
    .map(passwordRule => {
      ...
      const char1 = password.charAt(Number(min) - 1);
      const char2 = password.charAt(Number(max) - 1);

      return exactlyOneCharacterMatches(char1, char2, character);
    })
    .reduce((count, isValid) => isValid ? ++count : count);
}
